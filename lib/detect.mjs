...
export async function detectProject(cwd) {
  ...
  try {
    ...
    if (rootFiles.some(f => f === 'Jenkinsfile'))    signals.add('github-workflows')
    if (rootFiles.some(f => f === '.gitlab-ci.yml')) signals.add('github-workflows')
    if (rootFiles.some(f => f === 'dbt_project.yml'))signals.add('dbt')
    // Only match a directory literally named 'legacy', not files containing the word
    if (rootFiles.some(f => f === 'legacy' && fs.statSync(path.join(cwd, f)).isDirectory())) signals.add('legacy')
